...
struct TestFunkyVoidReturningTag;

// feature callables: move only
TEST(MakeFunkyVoidReturning, nonCopyableCallable_called_works) {
  // auto decorated_fn_all_covered = makeFunkyVoidReturning([](int) { PREVENT_CONSTANT_FOLDING(); });
  auto decorated_fn_miss_void_branch = makeFunkyVoidReturning([](int) {
  ...
  // decorated_fn_all_covered(argument);
  decorated_fn_miss_void_branch(argument);
}
