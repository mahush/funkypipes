...
#include "pipes/generic_make_pipe_tests.hpp"
...
// feature: chain breaking - fallback
TEST(MakePipe, compositionWithFailingCallbable_fallbackProvided_fallbackApplied) {
  auto breaking_lambda = []() -> std::optional<int> { return std::nullopt; };
  auto alternative_lambda = []() -> int { return 2; };
  auto forwarding_lambda = [](int arg) -> int { return arg; };

  auto pipe = makePipe(breaking_lambda, andThen(forwarding_lambda), orElse(alternative_lambda), forwarding_lambda);
