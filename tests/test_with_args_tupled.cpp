...
#include "funkypipes/details/with_args_tupled.hpp"

using funkypipes::details::withArgsTupled;

// Ensure that packing a tuple out of zero arguments works
TEST(WithArgsTupled, callableAccecptingEmptyTuple_calledWithoutArgs_works) {
  // given
  auto lambda = [](std::tuple<>) { return "output"; };
  auto packing_lambda = withArgsTupled(lambda);
...
// Ensure that packing a tuple out of a single argument works
TEST(WithArgsTupled, callableAccecptingTupleOfInt_calledWithInt_works) {
  // given
  auto lambda = [](std::tuple<int> value) { return std::get<int>(value); };
  auto packing_lambda = withArgsTupled(lambda);

  // when
  const int result = packing_lambda(1);
...
TEST(WithArgsTupled, callableAccecptingTupleOfIntAndString_calledWithIntAndString_works) {
  // given
...
TEST(WithArgsTupled, callableAcceptingTuple_calledWithLValueTuple_referenceIsPreserved) {
  // given
...
TEST(WithArgsTupled, callableAcceptingTuple_calledWithConstLValueTuple_referenceIsPreserved) {
  // given
...
TEST(WithArgsTupled, callableAcceptingTuple_calledWithRValueTuple_referenceIsPreserved) {
  // given
...
// Ensure that a single lvalue reference element gets forwarded as tuple while preserving element references
TEST(WithArgsTupled, callableAcceptingTupleOfInt_calledWithLValueInt_referenceIsPreserved) {
  // given
...
// Ensure that a single const lvalue reference element gets forwarded as tuple while preserving element references
TEST(WithArgsTupled, callableAcceptingTupleOfInt_calledWithConstLValueInt_referenceIsPreserved) {
  // given
...
// Ensure that a single rvalue reference element gets forwarded as tuple while preserving element references
TEST(WithArgsTupled, callableAcceptingTupleOfInt_calledWithRValueInt_referenceIsPreserved) {
  //
  // given
  //
  auto lambda = [](std::tuple<int&&> value) -> int&& { return std::get<0>(std::move(value)); };
  auto packing_lambda = withArgsTupled(lambda);
...
TEST(WithArgsTupled, callableAcceptingTupleOfLValueIntInt_calledWithLValueIntInt_referencesArePreserved) {
  // given
...
TEST(WithArgsTupled, callableAcceptingTupleOfConstLValueIntInt_calledWithConstLValueIntInt_referencesArePreserved) {
  // given
...
TEST(WithArgsTupled, callableAcceptingTupleOfRValueIntInt_calledWithRValueIntInt_referencesArePreserved) {
  // given
  auto lambda = [](std::tuple<int&&, int&&> arg) { return arg; };
  auto packing_lambda = withArgsTupled(lambda);
...
TEST(WithArgsTupled, callableReturningByValue_called_returnedByValue) {
  // given
  auto lambda = [](std::tuple<int> arg) -> int { return std::get<0>(arg); };
  auto packing_lambda = withArgsTupled(lambda);
