...
Your c++17 compatible toolbox for composing functions to facilitate a flow based programming style.
...
- **Chain Breaking**
  - *Failable Pipe Approach*: Any callable may return its result wrapped in std::optional. This way a callable is able to break the chain by providing a `std::nullopt`. In consequence the subsequent callables are not executed and the pipe returns a `std::nullopt` of the last chained callable's return value type. 
  - *Other approaches to come*: ...
