...
constexpr decltype(auto) unwrapOrForwardTuple(TTuple&& tuple) {
...
template <typename TTuple>
constexpr decltype(auto) collapseOrForwardTuple(TTuple&& tuple) {
  static_assert(IsTuple<std::decay_t<TTuple>>, "A tuple argument is required");
  ...
  if constexpr (size == 0) {
    ...
    namespace fpd = ::funkypipes::details::impl;
    return fpd::unwrapOrForwardTuple(std::forward<TTuple>(tuple));
  }
}
