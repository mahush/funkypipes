...
template <std::size_t... SelectedIdxs, typename... TElements>
auto recreateTupleFromIndices(const std::tuple<TElements...>& tuple) {
  return recreateTupleFromIndices(tuple, std::index_sequence<SelectedIdxs...>{});
}

template <std::size_t... SelectedIdxs, typename... TElements>
auto recreateTupleFromIndices(std::tuple<TElements...>&& tuple) {
  return recreateTupleFromIndices(std::move(tuple), std::index_sequence<SelectedIdxs...>{});
}
